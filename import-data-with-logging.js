...
async function importData() {
  ...
  try {
    ...
    // Check if blogs table exists in SQLite
    const blogTables = await new Promise((resolve, reject) => {
      sqliteDb.all("SELECT name FROM sqlite_master WHERE type='table' AND name='blogs'", [], (err, rows) => {
        if (err) reject(err);
        else resolve(rows);
      });
    });
    ...
    if (blogTables.length === 0) {
      console.error('Error: blogs table does not exist in SQLite database');
      return;
    }
    ...
    for (const blog of blogs) {
      try {
        console.log(`Processing blog ID: ${blog.id}, Title: ${blog.title}`);
        ...
        console.log(`Successfully processed blog: ${blog.title} (ID: ${blog.id})`);
      } catch (error) {
        console.error(`Error processing blog ${blog.id} (${blog.title}):`, error.message);
      }
    }
    ...
    console.log(`Found ${authors.length} authors in SQLite`);
    
    for (const author of authors) {
      try {
        console.log(`Processing author ID: ${author.id}, Name: ${author.name}`);
        const result = await pgClient.query(
          `INSERT INTO authors (
        ...
        console.log(`Successfully processed author: ${author.name} (ID: ${author.id})`);
      } catch (error) {
        console.error(`Error processing author ${author.id} (${author.name}):`, error.message);
      }
    }
    ...
    console.log(`Found ${blogAuthors.length} blog-author relationships in SQLite`);
    ...
    for (const rel of blogAuthors) {
      try {
        ...
        if (blogExists.rows.length > 0 && authorExists.rows.length > 0) {
          ...
          console.log(`Successfully processed relationship: blog ${rel.blog_id} <-> author ${rel.author_id}`);
        } else {
